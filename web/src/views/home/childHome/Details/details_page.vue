<template>
  <div v-if="this.$store.state.goodsList[good_id]">
    <!-- 图片 -->
    <div class="pic">
      <img :src="this.$store.state.goodsList[good_id].image" />
    </div>
    <!-- 文字 -->
    <div class="fonts">
      <!-- 标题 -->
      <div class="titles">
        <slot:titles>
          <h3>
            {{ this.$store.state.goodsList[good_id].introduce }}
          </h3>
        </slot:titles>
      </div>
      <!-- 价格 -->
      <div class="price">
        <slot:price>
          <h2>￥{{ this.$store.state.goodsList[good_id].price }}</h2>
        </slot:price>
      </div>
      <!-- 购买数量 -->
      <div class="num">
